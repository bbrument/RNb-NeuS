...
class NeuSRenderer:
    ...
    def render_core_normals(self,
        ...
        normal = sampled_normals * weights[:, :, None]
        normals = normal.sum(dim=1)
    ...
    def render_normals(self, rays_o, rays_d, near, far, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0):
        batch_size = len(rays_o)
        sample_dist = 2.0 / self.n_samples   # Assuming the region of interest is a unit sphere
        z_vals = torch.linspace(0.0, 1.0, self.n_samples)
        z_vals = near + (far - near) * z_vals[None, :]
        ...
        # Render core
        ret_fine = self.render_core_normals(rays_o,
                                    rays_d,
                                    z_vals,
                                    sample_dist,
                                    self.sdf_network,
    ...
    def render_normals2(self, rays_o, rays_d, near, far, l_directions, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0):
        batch_size = len(rays_o)
        sample_dist = 2.0 / self.n_samples   # Assuming the region of interest is a unit sphere
        z_vals = torch.linspace(0.0, 1.0, self.n_samples)
        z_vals = near + (far - near) * z_vals[None, :]
        ...
        # Render core
        ret_fine = self.render_core_normal_2(rays_o,
                                    rays_d,
                                    z_vals,
                                    sample_dist,
                                    self.sdf_network,
        ...
        directions = l_directions * torch.ones(l_directions.shape[0], batch_size, normal.shape[1], 3)
        a = torch.ones(l_directions.shape[0], batch_size, normal.shape[1], 3)
        b = relu((normal[None, :, :, :] * directions).sum(dim=-1, keepdim=True))
        weights = ret_fine['weights']
        color_fine = (a * b * weights[None, :, :, None]).sum(dim=2)
        albedo_fine = (a * weights[None, :, :, None]).sum(dim=2)
        weights_sum = weights.sum(dim=-1, keepdim=True)
        gradients = ret_fine['gradients']
        s_val = ret_fine['s_val'].reshape(batch_size, n_samples).mean(dim=-1, keepdim=True)
    ...
    def render_normal_integration_light_optimal(self, rays_o, rays_d, near, far, l_directions, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0,ind=-1,mask=-1):
        ...
        normal = ret_fine['sampled_normal']
        a = torch.ones(l_directions.shape[0], batch_size, normal.shape[1], 3)
        b = relu((normal[None, :, :, :] * l_directions).sum(dim=-1, keepdim=True))
        weights = ret_fine['weights']
        color_fine = (a * b * weights[None, :, :, None]).sum(dim=2)
        albedo_fine = (a * weights[None, :, :, None]).sum(dim=2)
        weights_sum = weights.sum(dim=-1, keepdim=True)
    ...
    def render_normal_integration_light_optimal_worelu(self, rays_o, rays_d, near, far, l_directions, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0,ind=-1,mask=-1):
        batch_size = len(rays_o)
        sample_dist = 2.0 / self.n_samples   # Assuming the region of interest is a unit sphere
        z_vals = torch.linspace(0.0, 1.0, self.n_samples)
        z_vals = near + (far - near) * z_vals[None, :]
        ...
        a = torch.ones(l_directions.shape[0], batch_size, normal.shape[1], 3)
        # b = relu((normal[None, :, :, :] * l_directions).sum(dim=-1, keepdim=True))
        b = (normal[None, :, :, :] * l_directions).sum(dim=-1, keepdim=True)
        weights = ret_fine['weights']
        color_fine = (a * b * weights[None, :, :, None]).sum(dim=2)
        albedo_fine = (a * weights[None, :, :, None]).sum(dim=2)
        weights_sum = weights.sum(dim=-1, keepdim=True)
